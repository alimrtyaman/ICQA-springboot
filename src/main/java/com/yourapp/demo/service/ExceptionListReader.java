    ...
    public Map<String, WaveAgg> readSecondSortingAgg(MultipartFile exceptionFile) {
        ...
        try (InputStream in = exceptionFile.getInputStream();
            ...
            if (waveIdx == null || shelvingIdx == null) {
                // Header bulunamazsa bunu görmen için patlatalım (sessiz 0 dönmesin)
                throw new RuntimeException("Cannot find required columns in Package-exception.xlsx. " +
                        "Found headers: " + idx.keySet());
            }
